...
if (process.env.VCAP_SERVICES) {
	...
	mongo = {
		...
		// to see more databases you need to set mongodb.admin to true or add databases
		// to the mongodb.auth list
		connectionString: process.env.MONGO_HOST ? '' : process.env.MONGO_PORT,
	};
}
...
module.exports = {
	mongodb: {
		...
		connectionString: process.env.MONGO_HOST || '',
		...
		/*
			>>>>  If you are using an admin mongodb account, or no admin account exists,
			fill out section below
	...
	options: {
		...
		// gridFSEnabled: if gridFSEnabled is set to 'true',
		// you will be able to manage uploaded files ( ak. grids, gridFS )
	...
	// Specify the default keyname that should be picked
	// from a document to display in collections list.
